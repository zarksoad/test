...
let staringTrip = () =>{
    let flag = true
    alert("Friday, Hildebrando arrives at El Dorado Airport. He wants to buy some food before the flight since he has a layover in Medellin in the early morning, and he would arrive in Macondo in the morning")
    while(flag){
    ...
    if(!isNaN(opcion)){
        if(opcion === 1) {
           ...
           characteristics.budget += 1000000;
            ...
            alert("Hildebrando bets, and only ends up with hist return ticket, sadly his vacation is over 😢")
            flag = false
    ...
    if(!isNaN(opcion)){
        if(opcion === 1) {
           alert("Hildebrando goes to a beautiful waterfall and take photos.")
            ...
            alert("He will reach a certain point, and will return alone, and at night gets lost 😨")
            alert("Vacations is over 😔")
            flag = false
        }    
    }
...
let results = ()=>{
    ...
    if(resultLive){
        alert("Hildebrado is alive")
    }
    else{
        alert("Hildebrado is Dead for his bad choises")
    }
...
function main(){
    ...
    convertVocal(word)
    data()
    alert("Finally , he can take a taxi")
    game()
    data()
